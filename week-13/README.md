> **Implemented setup:** All three implementations are in `task-1`, `task-2`, and `task-3`. Run `npm install`, then `npm run dev` here. The index links to every task. `npm run build` builds all pages.
