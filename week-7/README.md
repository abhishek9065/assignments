> **Implemented setup:** Configure `server/.env` from `.env.example`, then run `npm install` and `npm run dev` in `server`. Open http://localhost:3007 for the plain JS client. For React, run `npm install` and `npm run dev` in `client`; its proxy connects to port 3007. Create instructor and student accounts through the UI.
