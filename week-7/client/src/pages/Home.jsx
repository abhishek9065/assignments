...
export default function Home() {
  ...
  useEffect(() => {
    ...
    let active = true;
    Promise.all([
      api((session.role === 'admin' ? '/admin' : '/users') + '/courses', session),
      session.role === 'user'
        ? api('/users/purchasedCourses', session)
  ...
  const authenticate = async (values, action) => {
    setBusy(true);
    setMessage('');
    try {
  ...
  async function purchase(id) {
    ...
    try {
      ...
      setOwned((await api('/users/purchasedCourses', session)).purchasedCourses);
  ...
  async function saveCourse(event) {
    ...
    try {
      ...
      setCourses((await api('/admin/courses', session)).courses);
      setEditing(null);
      form.reset();
